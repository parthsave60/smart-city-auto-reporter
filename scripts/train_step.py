...
def main():
    parser = argparse.ArgumentParser(description="Single-epoch training step with resume")
    parser.add_argument("--epoch", type=int, required=True, help="Current epoch (1-based)")
    parser.add_argument("--total-epochs", type=int, default=30, help="Total target epochs")
    ...
    parser.add_argument("--output-dir", type=str, default=r"models\civic_issue_classifier_retrained")
    args = parser.parse_args()

    set_seed(42 + args.epoch)
    ...
    # Square-root inverse class weights
    ...
    norm_weights = np.clip(norm_weights, 0.3, 5.0)
    ...
    criterion = nn.CrossEntropyLoss(weight=class_weights_tensor, label_smoothing=0.03)
    ...
    # Load existing state if epoch > 1
    best_val_f1 = 0.0
    best_val_acc = 0.0
    best_epoch = 0
    patience_counter = 0
    ...
    if args.epoch > 1 and os.path.exists(state_path):
    ...
    # Save state for next epoch resume
    torch.save({
        "epoch": args.epoch,
        "model_state_dict": model.state_dict(),
    ...
    should_stop = (patience_counter >= args.patience) or (args.epoch >= args.total_epochs)

    duration = time.time() - t0
    print(f"\n--- EPOCH {args.epoch} FINISHED in {duration:.1f}s (Best: Epoch {best_epoch}, Val F1: {best_val_f1:.4f}) ---", flush=True)
    ...
    summary_data = {
        ...
        "per_class_recall": {cls: round(report[cls]["recall"] * 100, 2) for cls in TARGET_CLASSES}
    }
    ...
    print("\n========================================", flush=True)
    print("EPOCH_SUMMARY_START", flush=True)
    print(json.dumps(summary_data), flush=True)
    print("EPOCH_SUMMARY_END", flush=True)
    print("========================================\n", flush=True)
